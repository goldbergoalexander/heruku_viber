...
function wheather (response1){
	bot.sendMessage(response1,[new TextMessage('Привіт' + ' ' + ' Оберіть за якими параметрами Ви хочете отримати погодні умови ?')/*,new KeyboardMessage(keys)*/]).then(()=>{keyboard.get_keyboard_weather(response1);
	bot.onTextMessage(/За один день|oneday/, (message,response) => {
	bot.onLocationMessage(response1,[new LocationMessage(latitude, longitude)/*,new KeyboardMessage(keys)*/]).then(()=>{bot.sendMessage(response1,[new TextMessage('Привіт отже твоя локація' + response1.latitude + ' ' + response1.longitude)])  }).catch(err=>{throw err})
