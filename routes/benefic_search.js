...
//#################################################################### - search kved -  ############################################################
/*
if (msge.data==='benific'){
							
							bot.sendMessage(messages.chat.id, "для отримання данних введівть назву компаніії нажаль латиницею \ud83d\udc47 наприклад : 'EURO', 'Internatioonal'.. тощо  ").then(() => {
				 bot.once("text",(mess)=>{
                    bot.sendMessage(messages.chat.id, " інформація по бенефіціарам за Вашим запитом = > " + mess.text.toUpperCase() + " надійде якнайшвидше ")
...
MongoClient.connect(url1,{ useNewUrlParser: true }, function(err, db) {
  ...
  var dbo = db.db("work");
	...
	var fname = mess.chat.first_name.toUpperCase();
	var messageID = mess.chat.id;
...
dbo.collection('owner').find({$or:[{"statementGroups.beneficialOwnershipStatements.interestedParty.name": new RegExp(text1)},{"statementGroups.beneficialOwnershipStatements.entity.name": new RegExp(text1)}]}).toArray()
	.then(data => { 
	var datas = '';
for (var i= 0; i< data.length; i++) {
	var obj = Object.values(data[i]);
...
if (datas.length > 0) {
		...
		bot.sendMessage(messages.chat.id, ex = '``'+ '*'+ 'Привіт' + ' ' + '_'+fname + '_' + ' Ви обрали розділ Бенефіціари :' + '*' + '\n' +  "інформацІя згідно Вашого Запиту до боту @alldata : " + ' ' +  
        ' \n' + 
		'Імя ЮО Беніфіціара : ' + '*' +  datas[0] + '*'+
		'Країна : ' + '*' + datas[1] + '*'+
		'дата заснування : ' + '*' + datas[2]  + '*'+
